...
import type { ColumnId, TagKey, Task, Teammate } from './types';
...
export default function TaskModal({
  ...
  const {
    ...
    defaultValues: {
      ...
      assigneeId: task?.assigneeId ?? teammates[0]?.id ?? '',
  ...
  }, [task, defaultColumnId, teammates, reset]);
      ...
      <div className="fade-in w-full max-w-lg rounded-2xl bg-white shadow-xl dark:bg-card">
        ...
        <form onSubmit={handleSubmit(onSubmit)} noValidate>
          <div className="space-y-5 px-6 py-5">
            <div>
              <label className="mb-1.5 block text-sm font-medium text-foreground" htmlFor="task-title">
            ...
            <div>
              <label className="mb-1.5 block text-sm font-medium text-foreground" htmlFor="task-desc">
                Description
              </label>

              <p className="mb-1.5 text-xs text-muted-foreground">
                Optional - add context, links, or acceptance criteria.
              </p>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="mb-1.5 block text-sm font-medium text-foreground" htmlFor="task-tag">
                ...
                <select
                  ...
                  className={`w-full appearance-none rounded-lg border bg-white px-3 py-2.5 text-sm text-foreground transition-all focus:outline-none focus:ring-2 focus:ring-ring dark:bg-background ${
                    errors.tag ? 'border-red-400' : 'border-border focus:border-primary'
                  }`}
                ...
                {errors.tag && <p className="mt-1.5 text-xs text-red-500">{errors.tag.message}</p>}
              </div>

              <div>
                <label className="mb-1.5 block text-sm font-medium text-foreground" htmlFor="task-column">
                ...
                >
                  {COLUMN_ORDER.map((columnId) => (
                    <option key={`modal-col-${columnId}`} value={columnId}>
            ...
            <div>
              <label className="mb-1.5 block text-sm font-medium text-foreground" htmlFor="task-assignee">
              ...
              <div className="flex flex-wrap gap-2">
                {teammates.map((teammate) => (
                  <label key={`assignee-opt-${teammate.id}`} className="cursor-pointer">
                    <input
                      type="radio"
                      value={teammate.id}
                      className="sr-only"
                    ...
                    <AssigneeChip teammate={teammate} selected={selectedAssigneeId === teammate.id} />
                  </label>
                ))}
              </div>
          ...
          <div className="flex items-center justify-end gap-3 border-t border-border px-6 py-4">
            ...
            <button
              ...
              className="btn-primary flex items-center gap-2 rounded-lg px-5 py-2 text-sm font-semibold disabled:cursor-not-allowed disabled:opacity-70"
...
function AssigneeChip({ teammate, selected }: { teammate: Teammate; selected: boolean }) {
  return (
    <div
    ...
    >
      ...
      <span className="text-xs font-medium">{teammate.name.split(' ')[0]}</span>
    </div>
  );
}